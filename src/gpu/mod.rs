...
mod tests {
    ...
    fn gpu_mesh_extracts_supported_subset() {
        ...
        let report = crate::mesh::analyze_mesh_quality(&mesh);
        assert!(
            report.degenerate_triangle_count < report.triangle_count,
            "GPU mesh extractor should emit at least one non-degenerate triangle"
        );
        assert!(
            report.max_edge_length_mm > 0.0,
            "GPU mesh extractor should emit nonzero vertex positions"
        );
    }
    ...
    fn gpu_mesh_extracts_from_sampled_vertex_grid() {
        ...
        assert!(
            report.degenerate_triangle_count < report.triangle_count,
            "Sampled-grid GPU mesh extractor should emit at least one non-degenerate triangle"
        );
        assert!(
            report.max_edge_length_mm > 0.0,
            "Sampled-grid GPU mesh extractor should emit nonzero vertex positions"
        );
    }
