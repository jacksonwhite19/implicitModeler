...
use crate::fea::meshing::{TetMesh, voxel_tet_mesh};
use crate::fea::setup::{FEAConfig, FEASetup};
use crate::sdf::Sdf;
...
impl FEAPipeline {
    ...
    pub fn run(self, tx: std::sync::mpsc::Sender<FEAMessage>) {
        ...
        // ── Step 1: mesh ─────────────────────────────────────────────────────
        log!(
            "Meshing geometry at resolution {}…",
        ...
        let inp_content = match write_inp(
            &mesh,
            &self.setup,
        ...
        let grid_result = interpolate_idw(
            &mesh,
            &frd.displacement,
            &frd.von_mises,
            self.bounds_min,
        ...
        let max_displacement = grid_result
            .displacement
            .iter()
            .cloned()
            .fold(0.0f32, f32::max);
...
fn interpolate_idw(
    ...
    let pairs: Vec<(f32, f32)> = (0..total)
        ...
        .map(|idx| {
            ...
            let mut dists: Vec<(f32, usize)> = nodes
                .iter()
                .enumerate()
