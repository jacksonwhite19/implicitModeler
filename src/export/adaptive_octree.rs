...
pub const MIN_FEATURE_SIZE_MM: f32 = 0.8;
...
pub const REFINE_FEATURE_FLOOR: u32 = 1 << 9;
...
fn sample_cell_summary(sdf: &dyn Sdf, min: Vec3, max: Vec3) -> CellSampleSummary {
    ...
    let size = max - min;
    let half_diagonal = size.length() * 0.5;
    ...
    let center_distance = sdf.distance(center);
    let mut min_d = center_distance;
    let mut max_d = center_distance;
    ...
    let proximity_intersects = center_distance.abs() < half_diagonal * 1.2;
...
fn node_should_split(
    ...
    if summary.proximity_intersects {
    ...
    if near_surface && max_dim > MIN_FEATURE_SIZE_MM {
        flags |= REFINE_FEATURE_FLOOR;
    }
...
fn try_reserve_children(leaf_budget: &AtomicUsize) -> bool {
    leaf_budget
...
fn build_node_parallel(
    ...
    let (should_split, refinement_flags) = node_should_split(
        &summary,
        max_dim,
        depth,
        settings,
        leaf_budget.load(Ordering::Relaxed),
    );

    let children = if should_split && try_reserve_children(leaf_budget) {
        let bounds = child_bounds(min, max);
        let nodes: Vec<OctreeNode> = bounds
            .into_par_iter()
            .map(|(child_min, child_max)| {
                build_node_parallel(sdf, child_min, child_max, depth + 1, settings, leaf_budget)
            })
            .collect();
        Some(nodes)
    } else {
...
pub fn build_adaptive_octree(
    ...
    let leaf_budget = AtomicUsize::new(settings.max_leaf_cells.max(1));
    AdaptiveOctree {
        root: build_node_parallel(sdf, bounds_min, bounds_max, 0, settings, &leaf_budget),
    }
}
...
pub fn collect_leaf_cells(octree: &AdaptiveOctree) -> Vec<SurfaceLeafCell> {
    fn walk(node: &OctreeNode, out: &mut Vec<SurfaceLeafCell>) {
        match &node.children {
            Some(children) => {
                for child in children {
