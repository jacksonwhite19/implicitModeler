...
use super::{
    FieldHandle, MotorHandle, PointHandle, PropHandle, PropulsionHandle, SdfHandle, SectionHandle,
};
...
pub fn register_legacy_compat_functions(engine: &mut Engine) {
    use crate::sdf::aerospace::control_surfaces::{
        ControlHornSpec, HingeSpec, LinkageSpec, aileron as cs_aileron, elevator as cs_elevator,
        elevon as cs_elevon, estimate_half_span, rudder as cs_rudder,
    };
    ...
    engine.register_fn(
        ...
        |wing: SdfHandle, span_start: f64, span_end: f64, chord_fraction: f64| -> SdfHandle {
            ...
            let (span_start, span_end) = resolve_legacy_span_range(
                wing.0.as_ref(),
                span_start as f32,
                span_end as f32,
                half_span,
    ...
    engine.register_fn(
        ...
        |outer_r: f64, height: f64, insert_r: f64, insert_depth: f64| -> SdfHandle {
            ...
            let hole = Arc::new(Translate::new(
                hole as Arc<dyn crate::sdf::Sdf>,
                glam::Vec3::new(0.0, 0.0, z_off),
            ));
    ...
    engine.register_fn(
        ...
        |sdf: SdfHandle, upright: bool| -> rhai::Map {
            ...
            for z in 0..resolution {
                for y in 0..resolution {
                    for x in 0..resolution {
                        ...
                        let nx = sdf.0.distance(p + Vec3::X * normal_eps)
                            - sdf.0.distance(p - Vec3::X * normal_eps);
                        let ny = sdf.0.distance(p + Vec3::Y * normal_eps)
                            - sdf.0.distance(p - Vec3::Y * normal_eps);
                        let nz = sdf.0.distance(p + Vec3::Z * normal_eps)
                            - sdf.0.distance(p - Vec3::Z * normal_eps);
            ...
            let fraction = if surface_count > 0.0 {
                overhang_count as f64 / surface_count
            } else {
                0.0
            };
            let critical_fraction = if surface_count > 0.0 {
                critical_count as f64 / surface_count
            } else {
                0.0
            };
            map.insert(
                "max_angle_deg".into(),
                rhai::Dynamic::from(max_angle_deg as f64),
            );
            map.insert("fraction_over_45".into(), rhai::Dynamic::from(fraction));
            map.insert(
                "critical_fraction".into(),
                rhai::Dynamic::from(critical_fraction),
            );
            map.insert(
                "overhang_area_mm2".into(),
                rhai::Dynamic::from(overhang_area_mm2 as f64),
            );
            map
        },
    );
    engine.register_fn(
        ...
        |body: SdfHandle, settings: rhai::Map| -> SdfHandle {
    ...
    engine.register_fn(
        ...
        |body: SdfHandle,
         ...
         -> Result<SdfHandle, Box<rhai::EvalAltResult>> {
            ...
            Ok(SdfHandle(Arc::new(Union::new(
                result.part_a,
                result.part_b,
            ))))
        },
    ...
    engine.register_fn(
        ...
        |sdf: SdfHandle, axis_str: &str, pos: f64| -> PointHandle {
            ...
            PointHandle(crate::sdf::query::cross_section_centroid(
                sdf.0.as_ref(),
                axis,
                pos as f32,
            ))
    ...
    engine.register_fn(
        "gradient_field",
        |dx: f64, dy: f64, dz: f64| -> FieldHandle {
    ...
    engine.register_fn(
        "propulsion_setup",
        |m: MotorHandle, p: PropHandle, cells: f64, cap: f64| -> PropulsionHandle {
            use crate::aero::PropulsionSetup;
            PropulsionHandle(Arc::new(PropulsionSetup {
                motor: (*m.0).clone(),
                prop: (*p.0).clone(),
    ...
    engine.register_fn(
        ...
        |sections: rhai::Array| -> Result<SdfHandle, Box<rhai::EvalAltResult>> {
            ...
            let root_row = parse_row(&sections[0]).ok_or_else(|| -> Box<rhai::EvalAltResult> {
                "wing_from_sections: invalid root section".into()
            })?;
            let tip_row = parse_row(&sections[sections.len() - 1]).ok_or_else(
                || -> Box<rhai::EvalAltResult> { "wing_from_sections: invalid tip section".into() },
            )?;
            ...
            let sweep_off = sections[sections.len() - 1]
                ...
                .unwrap_or(0.0);
            let sweep_deg = if tip_y > 0.0 {
                (sweep_off / tip_y).atan().to_degrees()
            } else {
                0.0
            };
...
pub fn register_legacy_fea_compat_functions(
    ...
    use crate::fea::{
        FEAForceRegion, FEAMotorRegion, FEAPressureRegion, FEARegion, FEATorqueRegion,
    };
