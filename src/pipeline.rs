...
pub fn auto_bounds(sdf: &dyn Sdf) -> (Vec3, Vec3) {
    ...
    fn phased_bounds(
        ...
        let mut found = false;
        let surface_band = step.max_element() * 0.75;
        for &phase in phases {
            if let Some((lo, hi)) = scan_bounds(sdf, min, max, step, phase, surface_band) {
    ...
    // Bounds are inferred from interior samples, not exact surface crossings.
    // Keep at least one fine scan step of margin so small rounded shapes are not
    // clipped into flat-sided previews/exports.
    let pad = span * 0.08 + Vec3::splat(fine_step.max_element());
