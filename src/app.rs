...
const GEOMETRY_EDIT_DEBOUNCE: Duration = Duration::from_millis(2500);
...
pub struct App {
    ...
    conditioning_scheduler: crate::sdf::conditioning::BackgroundConditioningScheduler,
    pending_conditioning_job_id: Option<crate::sdf::conditioning::ConditioningJobId>,
    ...
    /// Set when a geometry edit should trigger script re-evaluation.
    dimensions_pending_eval: bool,
    /// Deadline for slider/live-edit debounce; None means evaluate on the next frame.
    dimensions_pending_eval_deadline: Option<Instant>,
...
impl App {
    pub fn new(_cc: &eframe::CreationContext) -> Self {
        ...
        let mut app = Self {
            ...
            conditioning_scheduler: crate::sdf::conditioning::BackgroundConditioningScheduler::new(
            ),
            pending_conditioning_job_id: None,
            resolution: 96,
            smooth_normals: true,
            ...
            dimensions_pending_eval_deadline: None,
...
"),
    ...
    fn execute_script(&mut self) {
        self.clear_pending_geometry_eval();
        ...
        if trimmed.is_empty() || trimmed.lines().all(|line| line.trim().starts_with("//")) {
            ...
            self.pending_conditioning_job_id = None;
            return;
        }
    ...
    fn apply_script_eval_success(&mut self, success: ScriptEvalSuccess) {
        ...
        let previous_sdf = self.current_sdf.clone();
        self.current_sdf = Some(Arc::clone(&success.sdf));
        self.current_mesh = None;
        ...
        self.enqueue_background_conditioning(previous_sdf, success.sdf);
    ...
    fn enqueue_background_conditioning(
        &mut self,
        previous_sdf: Option<Arc<dyn Sdf>>,
        canonical_sdf: Arc<dyn Sdf>,
    ) {
        let job_id = self
            .conditioning_scheduler
            .enqueue_after_edit(previous_sdf, canonical_sdf);
        self.pending_conditioning_job_id = Some(job_id);
        self.status_message = Some(format!(
            "Script rebuilt; SDF conditioning queued #{}",
            job_id.0
        ));
    }

    fn apply_background_conditioning_result(
        &mut self,
        result: crate::sdf::conditioning::BackgroundConditioningResult,
    ) {
        let start_mesh = std::time::Instant::now();
        self.current_sdf = Some(Arc::clone(&result.sdf));
        self.current_mesh = None;
        ...
        self.mesh_time_ms = Some(start_mesh.elapsed().as_secs_f64() * 1000.0);
        self.pending_conditioning_job_id = None;
        let elapsed = result
            .summary
            .elapsed_ms
            ...
            .unwrap_or_default();
        self.status_message = Some(format!(
            "SDF conditioning ready #{} ({:?}, {:?}{})",
            result.id.0, result.summary.cache_state, result.summary.update_mode, elapsed
        ));
    }

    fn poll_background_conditioning(&mut self, ctx: &egui::Context) {
        let Some(job_id) = self.pending_conditioning_job_id else {
            return;
        };

        let Some(summary) = self.conditioning_scheduler.job_summary(job_id) else {
            return;
        };

        match summary.state {
            crate::sdf::conditioning::ConditioningJobState::Queued
            | crate::sdf::conditioning::ConditioningJobState::Running => {
                self.status_message = Some(format!(
                    "SDF conditioning {:?} #{}",
                    summary.state, job_id.0
                ));
                ctx.request_repaint_after(std::time::Duration::from_millis(50));
            }
            crate::sdf::conditioning::ConditioningJobState::Ready => {
                if let Some(result) = self.conditioning_scheduler.take_latest_ready() {
                    if result.id == job_id {
                        self.apply_background_conditioning_result(result);
                    }
                }
            }
            crate::sdf::conditioning::ConditioningJobState::Failed => {
                self.pending_conditioning_job_id = None;
                self.status_message = summary
                    .error
                    .map(|error| format!("SDF conditioning failed: {error}"));
            }
            crate::sdf::conditioning::ConditioningJobState::Superseded => {
                self.pending_conditioning_job_id = None;
                self.status_message = Some(format!("SDF conditioning superseded #{}", job_id.0));
            }
        }
    }
    ...
    fn evaluate_and_render(&mut self, script: &str) {
        ...
        match scripting::evaluate_script_full(
            ...
            Ok(mut result) => {
                ...
                result.condition_auxiliary_geometry_for_backend();
                ...
                let sdf = result.canonical_sdf;
                let sdf_for_conditioning = Arc::clone(&sdf);
                ...
                self.enqueue_background_conditioning(previous_sdf, sdf_for_conditioning);
                self.error_message = None;
    ...
    fn apply_workflow_constraints(&mut self) -> usize {
        let applied = self
            ...
            .unwrap_or(0);
        if applied > 0 {
            self.request_immediate_geometry_eval();
        }
        applied
    }
    ...
    fn start_export_package_async(&mut self, output_dir: String) {
        ...
        std::thread::spawn(move || {
            tx.send(format!(
                "Building package mesh ({:.2} mm cells)…",
                quality.target_cell_size_mm()
            ))
            .ok();
            let mesh = crate::export::build_export_mesh(
    ...
    fn current_conditioning_cache(
        &self,
    ) -> Option<crate::sdf::conditioning::ConditionedCacheMetadata> {
        self.current_sdf
            .as_ref()
            .and_then(|sdf| sdf.metadata().conditioned_cache)
    }

    fn current_conditioning_job_summary(
        &self,
    ) -> Option<crate::sdf::conditioning::ConditioningJobSummary> {
        let job_id = self.pending_conditioning_job_id?;
        self.conditioning_scheduler.job_summary(job_id)
    }

    fn request_debounced_geometry_eval(&mut self) {
        self.dimensions_pending_eval = true;
        self.dimensions_pending_eval_deadline = Some(Instant::now() + GEOMETRY_EDIT_DEBOUNCE);
        self.status_message = Some(format!(
            "Geometry rebuild scheduled ({:.1}s debounce)",
            GEOMETRY_EDIT_DEBOUNCE.as_secs_f32()
        ));
    }

    fn request_immediate_geometry_eval(&mut self) {
        self.dimensions_pending_eval = true;
        self.dimensions_pending_eval_deadline = None;
    }

    fn clear_pending_geometry_eval(&mut self) {
        self.dimensions_pending_eval = false;
        self.dimensions_pending_eval_deadline = None;
    }

    fn pending_geometry_eval_remaining(&self) -> Option<Duration> {
        if !self.dimensions_pending_eval {
            return None;
        }
        let deadline = self.dimensions_pending_eval_deadline?;
        Some(deadline.saturating_duration_since(Instant::now()))
    }

    fn poll_pending_geometry_eval(&mut self, ctx: &egui::Context) {
        if !self.dimensions_pending_eval {
            return;
        }
        ...
        if let Some(remaining) = self.pending_geometry_eval_remaining() {
...
fn build_script_eval_success(
    mut result: scripting::ScriptResult,
    viewport_resolution: u32,
    eval_time_ms: f64,
    mut component_preview_parts: Vec<scripting::ComponentPreviewPart>,
) -> Result<ScriptEvalSuccess, String> {
    result.condition_auxiliary_geometry_for_backend();
    scripting::condition_component_preview_parts_for_backend(&mut component_preview_parts);
    let cg = result.center_of_gravity();
    let mass_points = result.mass_points;
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
                ...
                if resp.job_id == self.script_eval_job_id {
                    ...
                    match resp.outcome {
                        Ok(success) => {
                            ...
                            if self.pending_conditioning_job_id.is_none() {
        ...
        self.poll_background_conditioning(ctx);

        // Trigger re-eval when live geometry controls have settled.
        self.poll_pending_geometry_eval(ctx);
        ...
        egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("Export", |ui| {
                    ...
                    if ui
                        .add_enabled(has_sdf, egui::Button::new("Export STL…"))
                        .clicked()
                    {
                        if let Some(path) = rfd::FileDialog::new()
                            .add_filter("STL", &["stl"])
                            ...
                            .save_file()
                        {
                            self.start_export_async(
                                path.to_str().unwrap_or("output.stl").to_string(),
                                false,
                            );
                        }
                        ui.close_menu();
                    ...
                    if ui
                        .add_enabled(has_sdf, egui::Button::new("Export OBJ…"))
                        .clicked()
                    {
                        if let Some(path) = rfd::FileDialog::new()
                            .add_filter("OBJ", &["obj"])
                        ...
                        {
                            self.start_export_async(
                                path.to_str().unwrap_or("output.obj").to_string(),
                                true,
                            );
                        }
                        ui.close_menu();
                ...
                {
                    self.request_debounced_geometry_eval();
                }
        ...
        egui::TopBottomPanel::bottom("status_bar").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                } else if let Some(remaining) = self.pending_geometry_eval_remaining() {
                    ui.label(
                        egui::RichText::new(format!(
                            "Geometry rebuild in {:.1}s",
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if let Some(summary) = self.current_conditioning_job_summary() {
                        ui.label(
                            egui::RichText::new(format!(
                                "sdf {:?} #{}{}",
                                summary.state,
                    ...
                    } else if let Some(cache) = self.current_conditioning_cache() {
                        ui.label(
                            egui::RichText::new(format!(
                                "sdf {:?} g{} conf {:.2}",
                                cache.state, cache.generation, cache.confidence
                            ))
                            .small()
                            .weak(),
                        );
        ...
        egui::SidePanel::left("project_tree_panel")
            ...
            .show(ctx, |ui| {
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    let mut workflow_request_debounced_eval = false;
                    if let Some(cfg) = &mut self.workflow_config {
                        ...
                        ui.collapsing("Workflow Config", |ui| {
                            ...
                            if selected != cfg.manufacturing_preset {
                                ...
                                workflow_request_debounced_eval = true;
                            }
                    ...
                    if workflow_request_debounced_eval {
                        self.request_debounced_geometry_eval();
                    }
        ...
        egui::SidePanel::left("code_panel")
            ...
            .show(ctx, |ui| {
                ...
                if self.spine_editor_open {
                    let mut spine_length_changed = false;
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::DragValue::new(&mut self.spine_fuselage_length)
                                    .range(0.1f32..=500.0)
                                    .speed(0.1)
                                    .suffix(" u"),
                            )
                ...
                if let Some(ref grid) = self.current_sdf_grid {
                    ...
                    egui::Grid::new("mesh_stats")
                        ...
                        .show(ui, |ui| {
                            ...
                            if let Some(cache) = self.current_conditioning_cache() {
                                ui.label("SDF Cache:");
                                ui.label(format!(
                                    "{:?}, gen {}, conf {:.2}",
                                    cache.state, cache.generation, cache.confidence
                                ...
                                ui.label(format!(
                                    "{} @ {:.2} mm",
                                    cache.block_count, cache.grid_spacing
                                ));
                                ui.end_row();

                                ui.label("SDF Anchors:");
                                ui.label(format!(
                                    "{} total, {} projected",
                                    cache.anchor_count, cache.projection_anchor_count
                                ));
                                ui.end_row();
                            } else if let Some(summary) = self.current_conditioning_job_summary() {
                                ui.label("SDF Cache:");
                                ui.label(format!(
                                    "{:?} #{}, {:?}{}",
                                    summary.state,
