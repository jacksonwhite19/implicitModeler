...
impl Sdf for HaackNose {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        let radius = self.base_radius.abs();
        let bounds = SdfBounds::new(
            Vec3::new(0.0, -radius, -radius),
            Vec3::new(self.length.max(0.0), radius, radius),
        );
        SdfNodeMetadata::new("haack_nose")
            .with_support_bounds(bounds.clone())
            .with_feature_region(
                "haack_nose.profile",
                bounds,
                radius.min(self.length.abs() * 0.25).max(1.0e-3),
            )
            .with_f32_parameters([self.length, self.base_radius, self.c_parameter])
    }
}
...
impl Sdf for HaackTail {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        let radius = self.base_radius.abs().max(self.tip_radius.abs());
        let bounds = SdfBounds::new(
            Vec3::new(0.0, -radius, -radius),
            Vec3::new(self.length.max(0.0), radius, radius),
        );
        SdfNodeMetadata::new("haack_tail")
            .with_support_bounds(bounds.clone())
            .with_feature_region(
                "haack_tail.profile",
                bounds,
...
impl Sdf for TangentOgive {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        ...
        SdfNodeMetadata::new("tangent_ogive")
            .with_support_bounds(bounds.clone())
            .with_feature_region(
                "tangent_ogive.profile",
                bounds,
                radius.min(self.length.abs() * 0.25).max(1.0e-3),
            )
            .with_f32_parameters([self.length, self.base_radius])
    }
}
...
impl Sdf for EllipsoidNose {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        ...
        SdfNodeMetadata::new("ellipsoid_nose")
            .with_support_bounds(bounds.clone())
            .with_feature_region(
                "ellipsoid_nose.profile",
                bounds,
                radius.min(self.length.abs() * 0.25).max(1.0e-3),
            )
            .with_f32_parameters([self.length, self.base_radius])
...
mod tests {
    ...
    #[test]
    fn nose_tail_metadata_reports_support_bounds() {
        let nose = HaackNose::new(100.0, 15.0, 0.0);
        let tail = HaackTail::new(80.0, 12.0, 4.0, 0.0);
        let ogive = TangentOgive::new(60.0, 10.0);
        let ellipsoid = EllipsoidNose::new(50.0, 8.0);
        let nose_metadata = nose.metadata();
        let tail_metadata = tail.metadata();
        let ogive_metadata = ogive.metadata();
        let ellipsoid_metadata = ellipsoid.metadata();

        assert_eq!(
            nose_metadata.support_bounds,
            Some(SdfBounds::new(
                Vec3::new(0.0, -15.0, -15.0),
                Vec3::new(100.0, 15.0, 15.0)
            ))
        );
        assert_eq!(
            tail_metadata.support_bounds,
            Some(SdfBounds::new(
                Vec3::new(0.0, -12.0, -12.0),
                Vec3::new(80.0, 12.0, 12.0)
            ))
        );
        assert_eq!(ogive_metadata.node_kind, "tangent_ogive");
        assert_eq!(ellipsoid_metadata.node_kind, "ellipsoid_nose");
        assert!(
            nose_metadata
                .feature_ids
                .iter()
                .any(|id| id == "haack_nose.profile")
        );
        assert!(
            tail_metadata
                .feature_ids
                .iter()
                .any(|id| id == "haack_tail.profile")
        );
        assert_eq!(ogive_metadata.feature_regions.len(), 1);
        assert_eq!(ellipsoid_metadata.feature_regions.len(), 1);
        assert_ne!(nose_metadata.parameter_fingerprint, 0);
        assert_ne!(tail_metadata.parameter_fingerprint, 0);
    }
}
