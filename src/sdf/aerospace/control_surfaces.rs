...
use crate::sdf::primitives::{Cylinder, Plane, SdfBox};
...
impl HingeSpec {
    pub fn simple_gap(gap_width: f32) -> Self {
        Self {
            hinge_type: HingeType::SimpleGap,
            hinge_radius: 0.0,
            gap_width,
        }
    }
...
impl ControlHornSpec {
    pub fn default_upper(horn_height: f32, hole_offset: f32, span_fraction: f32) -> Self {
        Self {
            ...
            hole_diameter: 2.0,
            hole_offset,
            position_span_fraction: span_fraction,
            side: HornSide::Upper,
        }
    }
...
impl LinkageSpec {
    ...
    pub fn horn(spec: ControlHornSpec) -> Self {
        Self {
            control_horn: Some(spec),
            pushrod_slot: None,
        }
    }
}
...
pub fn estimate_half_span(sdf: &dyn Sdf) -> f32 {
    ...
    loop {
        ...
        let has_material = [-20.0_f32, 0.0, 20.0, 50.0, 100.0]
            .iter()
...
pub fn build_control_surface(spec: &ControlSurface) -> ControlSurfaceResult {
    ...
    let aft_plane: Arc<dyn Sdf> = Arc::new(Plane {
        normal: Vec3::NEG_X,
        distance: -x_hinge,
    });
    // Span lower: y > y_start → Plane normal=-Y, distance=-y_start
    let span_lo: Arc<dyn Sdf> = Arc::new(Plane {
        normal: Vec3::NEG_Y,
        distance: -y_start,
    });
    // Span upper: y < y_end → Plane normal=+Y, distance=y_end
    let span_hi: Arc<dyn Sdf> = Arc::new(Plane {
        normal: Vec3::Y,
        distance: y_end,
    });
    ...
    let (control_surface, modified_parent): (Arc<dyn Sdf>, Arc<dyn Sdf>) =
        match spec.hinge.hinge_type {
            HingeType::SimpleGap => {
            ...
            HingeType::Rounded => {
                ...
                let hinge_cyl: Arc<dyn Sdf> =
                    Arc::new(Translate::new(cyl_y, Vec3::new(x_hinge, y_mid, 0.0)));
                ...
                let socket: Arc<dyn Sdf> =
                    Arc::new(Translate::new(sock_y, Vec3::new(x_hinge, y_mid, 0.0)));
    ...
    if let Some(horn_spec) = &spec.linkage.control_horn {
        ...
        let z_dir = match horn_spec.side {
            HornSide::Upper => 1.0_f32,
            HornSide::Lower => -1.0,
        };
        ...
        let hole: Arc<dyn Sdf> = Arc::new(Translate::new(
            hole_rotated,
            Vec3::new(x_hinge + 1.0, y_horn, z_hole),
        ));
...
pub fn wing_with_ailerons(
    ...
    let neg_result = aileron(
        pos_result.modified_parent.clone(),
        -y_e,
        -y_s,
        chord_fraction,
        hinge,
    ...
    (
        pos_result.control_surface,
        neg_result.control_surface,
        neg_result.modified_parent,
    )
}
