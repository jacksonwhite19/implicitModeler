...
pub fn compute_tail_volumes(
    ...
    let vht = if s_w > 0.0 && mac_w > 0.0 {
        (s_ht * l_ht) / (s_w * mac_w)
    } else {
        0.0
    };
    let vvt = if s_w > 0.0 && b_w > 0.0 {
        (s_vt * l_vt) / (s_w * b_w)
    } else {
        0.0
    };
    ...
    let h_msg = if vht >= 0.40 {
        ...
        format!(
            "VHT={:.3} — marginal, consider increasing H-tail area or moment arm",
            vht
        )
    } else {
        format!(
            "VHT={:.3} — inadequate, increase H-tail area to {:.0}mm²",
            vht,
            0.30 * s_w * mac_w / l_ht.max(1.0)
        )
    };
    ...
    let v_msg = if vvt >= 0.04 {
        ...
        format!(
            "VVT={:.3} — inadequate, increase V-tail area to {:.0}mm²",
            vvt,
            0.02 * s_w * b_w / l_vt.max(1.0)
        )
    };
