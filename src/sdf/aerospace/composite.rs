...
impl Sdf for CompositeSdf {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        let outer_metadata = self.outer_mold_sdf.metadata();
        let outer_quality = outer_metadata.bound_quality;
        let support_bounds = outer_metadata.support_bounds.clone();
        let mut metadata = SdfNodeMetadata::new("composite_layup")
            .with_dependency("outer_mold", outer_metadata)
            .inherit_dependency_feature_ids();
        metadata.set_support_bounds(support_bounds, outer_quality);
        metadata
    }
}
