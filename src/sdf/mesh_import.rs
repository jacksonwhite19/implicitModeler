...
fn build_bvh_recursive(mesh: &TriangleMesh, tris: &mut [u32], nodes: &mut Vec<BvhNode>) -> i32 {
    ...
    let centroid_of = |t: u32| -> f32 {
        ...
        let v = (mesh.vertices[a as usize] + mesh.vertices[b as usize] + mesh.vertices[c as usize])
            / 3.0;
        match axis {
            0 => v.x,
            1 => v.y,
    ...
    let split = tris
        .partition_point(|&t| centroid_of(t) < mid_val)
        .max(1)
        .min(tris.len() - 1);
...
fn nearest_recursive(
    ...
    if n.aabb.dist_sq_to_point(p) >= best {
        return (best, 0);
    }
...
impl MeshSdfApprox {
    ...
    fn sample(&self, p: Vec3) -> f32 {
        ...
        let get = |ix: isize, iy: isize, iz: isize| -> f32 {
            ...
            self.grid
                [ix as usize + iy as usize * APPROX_RES + iz as usize * APPROX_RES * APPROX_RES]
        };
...
impl Sdf for MeshSdfApprox {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        SdfNodeMetadata::new("mesh_sdf_approx").with_estimated_bounds(SdfBounds::new(
            self.bounds_min,
            self.bounds_min + self.voxel_sz * APPROX_RES as f32,
        ))
    }
}
...
impl Sdf for MeshSdfAccurate {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        SdfNodeMetadata::new("mesh_sdf_accurate").with_estimated_bounds(SdfBounds::new(
            self.sign_bmin,
            self.sign_bmin + self.sign_vsz * SIGN_RES as f32,
        ))
    }
}
...
impl Sdf for MeshSdf {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        let active_metadata = match self.mode {
            MeshSdfMode::Approximate => self.approx.metadata(),
            MeshSdfMode::Accurate => {
                if let Some(ref acc) = self.accurate {
                    acc.metadata()
                } else {
                    self.approx.metadata()
                }
            }
        };
        let support_bounds = active_metadata.support_bounds.clone();
        let active_quality = active_metadata.bound_quality;
        let mut metadata = SdfNodeMetadata::new("mesh_sdf")
            .with_dependency("active", active_metadata)
            .with_approximate_bounds();
        metadata.set_support_bounds(support_bounds, active_quality);
        metadata
    }
}
...
mod tests {
    ...
    #[test]
    fn mesh_sdf_metadata_reports_import_bounds() {
        let data = cube_stl_bytes();
        let mesh = parse_stl(&data).unwrap();
        let sdf = MeshSdf::new(Arc::new(mesh));

        let metadata = sdf.metadata();
        let bounds = metadata.support_bounds.unwrap();

        assert_eq!(metadata.node_kind, "mesh_sdf");
        assert!(metadata.bounds_are_approximate);
        assert_eq!(metadata.dependencies.len(), 1);
        assert!((bounds.min.x + 1.6).abs() < 1e-5);
        assert!((bounds.max.x - 1.6).abs() < 1e-5);
    }
