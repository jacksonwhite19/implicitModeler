...
use super::Sdf;
use super::aerospace::{Section2D, SectionBounds2D};
...
impl SplineProfile {
    ...
    pub fn sample(&self, n: usize) -> Vec<Vec2> {
        ...
        if nc == 1 {
            return vec![self.control_points[0]; n];
        }
        (0..n)
            .map(|i| {
    ...
    fn eval_at(&self, t: f32) -> Vec2 {
        ...
        let p0 = if self.sharp_points.contains(&i0) {
            p1
        } else {
            p0
        };
        let p3 = if self.sharp_points.contains(&i1) {
            p2
        } else {
            p3
        };
...
impl Section2D for SplineProfile {
    fn distance_2d(&self, point: Vec2) -> f32 {
        ...
        let sign = if winding_number(&poly, point) != 0 {
            -1.0
        } else {
            1.0
        };
    ...
    fn bounds_2d(&self) -> Option<SectionBounds2D> {
        let samples = self.sample(128);
        SectionBounds2D::from_points(&samples).map(|bounds| {
            let padding = bounds.max_extent().max(1.0) * 0.02;
    ...
    fn lerp_to(&self, other: &dyn Section2D, t: f32) -> Arc<dyn Section2D> {
        ...
        let n = 64.max(self.control_points.len()).max(
            other
                .as_any()
        ...
        let lerped: Vec<Vec2> = self_pts
            .iter()
            .zip(other_pts.iter())
            .map(|(&a, &b)| a.lerp(b, t))
            .collect();

        Arc::new(SplineProfile {
            control_points: lerped,
            sharp_points: vec![],
            point_roles: vec![PointRole::Free; n],
        })
    }
...
impl Sdf for SplineProfile {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        SdfNodeMetadata::new("spline_profile_preview").with_approximate_bounds()
    }
}
...
mod tests {
    ...
    fn test_circle_profile_outside() {
        ...
        assert!(
            d > 0.0,
            "point outside circle should be positive, got {}",
            d
        );
    }
    ...
    fn test_sdf_3d_extrusion() {
        ...
        assert!(
            p.distance(Vec3::new(0.0, 5.0, 0.0)) < 0.0,
            "origin in XZ should be inside"
        );
        assert!(
            p.distance(Vec3::new(3.0, 0.0, 0.0)) > 0.0,
            "far XZ point should be outside"
        );
    }
}
