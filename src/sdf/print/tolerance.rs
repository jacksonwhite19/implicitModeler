...
impl Sdf for ToleranceCompensated {
    ...
    fn metadata(&self) -> SdfNodeMetadata {
        ...
        let support_bounds = child_metadata
            .support_bounds
            .as_ref()
            .map(|bounds| bounds.expanded(max_offset));
        let mut metadata = SdfNodeMetadata::new("tolerance_compensated")
            .with_dependency("child", child_metadata)
            .inherit_dependency_feature_ids()
            .with_approximate_bounds();
        metadata.set_support_bounds(support_bounds, BoundQuality::Conservative);
        metadata
    }
}
...
mod tests {
    ...
    #[test]
    fn tolerance_metadata_delegates_child_bounds() {
        let sphere: Arc<dyn Sdf> = Arc::new(Sphere::new(10.0));
        let comp = ToleranceCompensated::new(sphere, default_settings());

        let metadata = comp.metadata();

        assert_eq!(metadata.node_kind, "tolerance_compensated");
        assert!(metadata.bounds_are_approximate);
        assert_eq!(metadata.dependencies.len(), 1);
        assert_eq!(metadata.support_bounds.unwrap().max, Vec3::splat(10.15));
    }
}
