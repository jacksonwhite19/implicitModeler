...
pub fn compute_model_properties(grid: &SdfGrid) -> (f32, f32, Vec3) {
    ...
    let rows: Vec<(f64, f64, f64, f64, f64)> = (0..res)
        .into_par_iter()
        .map(|iz| {
            let mut vol = 0.0f64;
            let mut sa = 0.0f64;
    ...
    let (vol, sa, cx, cy, cz) = rows
        .iter()
        .fold((0.0f64, 0.0f64, 0.0f64, 0.0f64, 0.0f64), |a, b| {
