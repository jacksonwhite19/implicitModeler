...
impl Default for SectionUniforms {
    fn default() -> Self {
        Self {
            ...
            plane_a_axis: 0,
            plane_a_position: 0.0,
            plane_a_flip: false,
            plane_b_enabled: false,
            plane_b_axis: 0,
            plane_b_position: 0.0,
            plane_b_flip: false,
        }
    }
}
...
impl RaymarchRenderer {
    ...
    pub fn upload_grid(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, grid: &Arc<SdfGrid>) {
    ...
    fn rebuild_bind_group(&mut self, device: &wgpu::Device) {
        let Some(ref sdf_view) = self.sdf_view else {
            return;
        };
        let thickness_view = self
            .thickness_view
            .as_ref()
            .unwrap_or(&self.dummy_thickness_view);
    ...
    pub fn update_uniforms(
        ...
        let Some(ref grid) = self.last_grid else {
            return;
        };

        let span = (grid.bounds_max - grid.bounds_min).max(Vec3::splat(1.0));
