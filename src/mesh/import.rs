...
use crate::mesh::quality::analyze_triangle_mesh_quality;
use glam::Vec3;
...
pub fn validate_mesh(mesh: &TriangleMesh) -> MeshValidationResult {
    ...
    MeshValidationResult {
        degenerate_count: report.degenerate_triangle_count as u32,
        has_degenerate: report.degenerate_triangle_count > 0,
        has_non_manifold: report.non_manifold_edge_count > 0,
        has_open_boundary: report.boundary_edge_count > 0,
    }
}
...
pub fn parse_stl(data: &[u8]) -> Result<TriangleMesh, String> {
    ...
    if is_binary_stl(data) {
        ...
        let text =
            std::str::from_utf8(data).map_err(|e| format!("STL: UTF-8 decode failed: {}", e))?;
