...
fn build_tree(
    ...
    if !mass_points.is_empty() {
        let children: Vec<TreeNode> = mass_points
            .iter()
            .map(|mp| TreeNode::Leaf {
    ...
    if !profile_names.is_empty() {
        ...
        sorted.sort();
        let children: Vec<TreeNode> = sorted
            .iter()
            .map(|name| TreeNode::Leaf {
    ...
    if !ref_points.is_empty() {
        let children: Vec<TreeNode> = ref_points
            .iter()
            .map(|rp| {
                ...
                TreeNode::Leaf {
                    ...
                    label: format!(
                        "{} ({:.1}, {:.1}, {:.1})",
                        rp.name, rp.position.x, rp.position.y, rp.position.z
                    ),
                    icon: "◉",
                    color,
                    script_name: None,
...
pub fn show_project_tree(ui: &mut egui::Ui, tree: &mut ProjectTree) -> TreeInteraction {
    let mut interaction = TreeInteraction {
        jump_to_name: None,
        start_rename: None,
        confirmed_rename: None,
    };
            ...
            .show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ok_clicked && !rs.buffer.is_empty() && rs.buffer != rs.old_name {
                        confirm_rename =
                            Some((rs.old_name.clone(), rs.buffer.clone(), rs.node_id.clone()));
                    }
                ...
                if rs.needs_confirm {
                    ...
                    ui.colored_label(
                        Color32::YELLOW,
                        format!(
                            "⚠ Name appears {} times. All will be replaced.",
                            rs.occurrence_count
                        ),
                    );
                }
