...
use crate::ui::script_variable_detector::{
    DetectedVariable, DetectionType, detect_script_variables, extract_literal_at, infer_unit,
    rewrite_literal,
};
use crate::undo::{
    AppState, DimensionDeleteCommand, DimensionEditCommand, ScriptTextCommand, UndoHistory,
};
...
pub fn show_dimensions_panel(
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if ui.small_button("+").clicked() {
                ...
                let name = loop {
                    ...
                    if !state.dimensions.contains_key(&n) {
                        break n;
                    }
                    idx += 1;
                };
    ...
    if keys.is_empty() {
        ui.label(
            RichText::new("No dimensions — press + to add")
                .color(Color32::GRAY)
                .size(11.0),
        );
    }
    ...
    if !keys.is_empty() {
        egui::Grid::new("dims_grid")
            .num_columns(3)
            .spacing([4.0, 2.0])
            .striped(true)
            .show(ui, |ui| {
                for (row, name) in keys.iter().enumerate() {
                    let Some(&value) = state.dimensions.get(name.as_str()) else {
                        ui.end_row();
                        continue;
                    };
                    ...
                    if ds.editing_name == Some(row) {
                        ...
                        if resp.lost_focus() || ui.input(|i| i.key_pressed(egui::Key::Enter)) {
                            ...
                            if is_valid && new_name != *name {
                                history.execute(
                                    Box::new(DimensionEditCommand::new_rename(
                                        name.clone(),
                                        value,
                                        new_name,
                                        row,
                    ...
                    if ds.slider_row == Some(row) {
                            ...
                            if let Some(before) = ds.drag_start.take() {
                                if before != v {
                                    history.push_executed(Box::new(
                                        DimensionEditCommand::new_value(
                                            name.clone(),
                                            before,
                                            v,
                                            row,
                        ...
                        if resp.lost_focus() || ui.input(|i| i.key_pressed(egui::Key::Enter)) {
                            if let Ok(new_val) = buf.trim().parse::<f64>() {
                                if new_val != value {
                                    history.execute(
                                        Box::new(DimensionEditCommand::new_value(
                                            name.clone(),
                                            value,
                                            new_val,
                                            row,
                        ...
                        if resp.double_clicked() {
                            ...
                            ds.slider_range =
                                (value - value.abs().max(1.0), value + value.abs().max(1.0));
                        }
        ...
        if let Some(row) = delete_index {
            if let Some(name) = keys.get(row) {
                if let Some(&value) = state.dimensions.get(name.as_str()) {
                    history.execute(
                        Box::new(DimensionDeleteCommand {
                            name: name.clone(),
                            value,
                            index: row,
                        }),
...
fn show_script_variables_section(
    ...
    for cell in &cells {
        let section_vars: Vec<&DetectedVariable> = vars
            .iter()
...
fn render_sv_row(
    ...
    // Auto-range.
    let (min, max) = *ds
        .variable_slider_ranges
        .entry(key.clone())
        .or_insert_with(|| {
            if var.value > 0.0 {
                (var.value * 0.1, var.value * 3.0)
            } else if var.value < 0.0 {
                (var.value * 3.0, var.value * 0.1)
            } else {
                (-100.0, 100.0)
            }
        });
    ...
    let new_val_from_te = ui
        .horizontal(|ui| -> Option<f64> {
    ...
    slider_resp.context_menu(|ui| {
        ...
        ui.menu_button("Set Range...", |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .add(egui::TextEdit::singleline(&mut min_buf).desired_width(60.0))
                    .lost_focus()
                {
                    if let Ok(v) = min_buf.trim().parse::<f64>() {
                        entry.0 = v;
                    }
                }
            });
            ui.horizontal(|ui| {
                ...
                if ui
                    .add(egui::TextEdit::singleline(&mut max_buf).desired_width(60.0))
                    .lost_focus()
                {
                    if let Ok(v) = max_buf.trim().parse::<f64>() {
                        entry.1 = v;
                    }
                }
            });
...
fn convert_to_variable(state: &mut AppState, var: &DetectedVariable) {
    ...
    let byte_start = if var.col_start < char_count {
        char_indices[var.col_start].0
    } else {
        target_line.len()
    };
    let byte_end = if var.col_end < char_count {
        char_indices[var.col_end].0
    } else {
        target_line.len()
    };
