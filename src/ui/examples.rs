...
impl ExampleScript {
    pub fn maturity(&self) -> Maturity {
        ...
        for func_name in self.related_functions {
            if let Some(doc) = crate::ui::help_data::FUNCTION_DOCS
                .iter()
                .find(|doc| doc.name == *func_name)
            {
                match crate::ui::help_data::function_status(doc) {
                    crate::ui::help_data::FunctionStatus::Legacy => return Maturity::Legacy,
                    crate::ui::help_data::FunctionStatus::Experimental => {
...
mod tests {
        ...
        for ex in EXAMPLES {
            let has_legacy = ex.related_functions.iter().any(|name| {
                crate::ui::help_data::FUNCTION_DOCS
                    ...
                    .map(|doc| {
                        crate::ui::help_data::function_status(doc)
                            == crate::ui::help_data::FunctionStatus::Legacy
                    })
                    .unwrap_or(false)
            });
            ...
            match ex.maturity() {
                Maturity::Legacy => assert!(
                    has_legacy,
                    "{} should only be legacy if a related function is legacy",
                    ex.id
                ),
                Maturity::Experimental => assert!(
                    !has_legacy && has_experimental,
                    "{} should only be experimental if a related function is experimental",
                    ex.id
                ),
                Maturity::Stable => assert!(
                    !has_legacy && !has_experimental,
                    "{} should only be stable if all documented related functions are stable",
                    ex.id
                ),
            }
        }
    ...
    fn at_least_three_gold_standard_aircraft_examples_exist() {
        ...
        assert!(
            gold_examples.len() >= 3,
            "Expected at least 3 gold-standard complete-aircraft examples"
        );
        assert!(
            gold_examples
                .iter()
                .all(|ex| ex.maturity() == Maturity::Stable)
        );
    }
}
