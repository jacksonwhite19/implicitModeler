...
use crate::library::thumbnail::{THUMB_H, THUMB_W};
...
pub fn show_library_panel(
    ...
    if state.open {
        for comp in manager.components.iter_mut() {
            if comp.thumbnail_state == ThumbnailState::NotGenerated {
                ...
                let preview_call = comp.metadata.preview_fn.clone().or_else(|| {
                    comp.exported_functions.first().map(|sig| {
                        let args = sig
                            .params
                            .iter()
                            .map(|_| "0.0")
    ...
    let header = ui.collapsing("Library", |ui| {
        ...
        let filtered: Vec<usize> = manager
            .components
            .iter()
            .enumerate()
            .filter(|(_, c)| {
        ...
        if filtered.is_empty() {
            ui.label(
                RichText::new("No components found")
                    .color(Color32::GRAY)
                    .italics(),
            );
            return;
...
pub fn find_library_references(
    script: &str,
