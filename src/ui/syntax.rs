...
const KEYWORDS: &[&str] = &[
    "let", "if", "else", "for", "while", "fn", "return", "true", "false", "break", "continue",
    "loop", "in", "switch", "do",
];
...
const MATH_FNS: &[&str] = &["to_rad", "to_deg", "clamp", "lerp"];
...
fn word_color(word: &str) -> Color32 {
    ...
    if TRANSFORMS.contains(&word) {
        return C_TRANSFORM;
    }
    if PATTERNS.contains(&word) {
        return C_PATTERN;
    }
    if AEROSPACE.contains(&word) {
        return C_AEROSPACE;
    }
    ...
    if FEA_FNS.contains(&word) {
        return C_FEA;
    }
    if MATH_FNS.contains(&word) {
        return C_TRANSFORM;
    } // treat as transform-ish
    C_IDENT
}
