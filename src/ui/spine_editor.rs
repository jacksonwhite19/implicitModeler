...
pub fn show_spine_editor(
    ...
    ui.horizontal(|ui| {
        ...
        for target in [
            ...
            let btn = egui::RichText::new(target.label()).color(if active {
                target.color()
            } else {
                egui::Color32::GRAY
            });
    ...
    ui.horizontal(|ui| {
        ui.label(
            egui::RichText::new(state.active_target.label())
                .color(color)
                .strong(),
        );
        let pt_count = curve.as_ref().map(|c| c.control_points.len()).unwrap_or(0);
        ui.label(
            egui::RichText::new(format!("{} pts", pt_count))
                .weak()
                .small(),
        );
    ...
    for (other_target, other_curve) in all_curves(splines) {
        if other_target == state.active_target {
            continue;
        }
        if let Some(c) = other_curve {
            draw_curve(
                &painter,
                c,
                other_target.color().linear_multiply(0.3),
                fuselage_length,
                &m2c,
            );
        }
        ...
        if let (Some(di), Some(ptr)) = (state.dragging_point, pointer) {
            if rect.contains(ptr) {
                ...
                if di < c.control_points.len() {
                    ...
                    if let Some(new_idx) = c
                        .control_points
                        .iter()
                        .position(|&[x, y]| (x - new_x).abs() < 1e-4 && (y - new_y).abs() < 1e-4)
                    {
    ...
    if response.secondary_clicked() {
        if let Some(idx) = hovered {
            ...
            if c.control_points.len() > 1 {
                ...
                if state.selected_point == Some(idx) {
                    state.selected_point = None;
                }
                changed = true;
            }
    ...
    for (i, &[x, y]) in curve_pts.iter().enumerate() {
        ...
        painter.circle_stroke(
            cp,
            handle_r,
            egui::Stroke::new(1.0, egui::Color32::from_rgb(40, 45, 55)),
        );
    }
    ...
    // ── Segment distance annotations on canvas ────────────────────────────
    {
        ...
        for w in curve_pts.windows(2) {
            ...
            painter.line_segment(
                [
                    egui::Pos2::new(pa.x, pa.y - tick),
                    egui::Pos2::new(pa.x, pa.y + tick),
                ],
                egui::Stroke::new(0.8, ann_col),
            );
            painter.line_segment(
                [
                    egui::Pos2::new(pb.x, pb.y - tick),
                    egui::Pos2::new(pb.x, pb.y + tick),
                ],
                egui::Stroke::new(0.8, ann_col),
            );
            ...
            ui.horizontal(|ui| {
                ...
                let xr = ui.add(
                    egui::DragValue::new(&mut x_val)
                        .range(0.0..=fuselage_length)
                        .speed(0.005)
                        .suffix(" u"),
                );
                ui.label("Y:");
                let yr = ui.add(egui::DragValue::new(&mut y_val).speed(0.005).suffix(" u"));
...
fn draw_curve(
    ...
    let polyline: Vec<egui::Pos2> = (0..=samples)
        .filter_map(|i| {
...
fn draw_spine_grid(
    ...
    for xi in 0..=n_x {
        ...
        painter.text(
            egui::Pos2::new(p0.x + 2.0, rect.bottom() - 12.0),
    ...
    while y <= y_hi + y_step * 0.1 {
        ...
        painter.line_segment(
            [p0, p1],
            egui::Stroke::new(if on_zero { 1.2 } else { 0.8 }, col),
        );
        painter.text(
            egui::Pos2::new(rect.left() + 3.0, p0.y - 10.0),
...
fn get_curve<'a>(
    splines: &'a LongitudinalSplines,
    target: SpineEditorTarget,
) -> Option<&'a AxisCurve> {
    match target {
...
fn get_curve_mut<'a>(
    splines: &'a mut LongitudinalSplines,
    target: SpineEditorTarget,
) -> &'a mut Option<AxisCurve> {
    match target {
