...
use crate::aero::{flight_condition::FlightCondition, polars::PolarDatabase};
use crate::sdf::Sdf;
...
pub fn compute_neutral_point(
    ...
    let v_fuse_m3 =
        (fuse_bbox.size.x * fuse_bbox.size.y * fuse_bbox.size.z) / (1000.0_f32.powi(3)) * 0.6;
    ...
    let num = wing_ac_x * s_wing_m2 * a_w + htail_ac_x * s_htail_m2 * a_t * eta_t * (1.0 - d_eps);
    let den = s_wing_m2 * a_w + s_htail_m2 * a_t * eta_t * (1.0 - d_eps);

    let x_np_raw = if den.abs() > 1e-12 {
        num / den
    } else {
        wing_ac_x
    };
...
pub fn compute_trim(
    ...
    let l_tail_mm = (np_result.htail_ac_x_mm - np_result.wing_ac_x_mm)
        .abs()
        .max(1.0);
    ...
    let polar = polar_db
        .get_interpolated("NACA 0012", re_wing)
...
mod tests {
    ...
    fn test_np_wing_only() {
        ...
        assert!(
            np.neutral_point_x_mm > 0.0,
            "NP x should be positive, got {}",
            np.neutral_point_x_mm
        );
        // Wing contribution should be significant.
        assert!(
            np.wing_ac_contribution > 0.3,
            "Wing should have significant contribution, got {}",
            np.wing_ac_contribution
        );
    }
    ...
    fn test_tail_moves_np_aft() {
        ...
        assert!(
            np_lg.neutral_point_x_mm >= np_sm.neutral_point_x_mm - 5.0,
            "Large tail NP should be >= small tail NP: sm={:.1} lg={:.1}",
            np_sm.neutral_point_x_mm,
            np_lg.neutral_point_x_mm
        );
        // Both NPs should be aft of wing AC (25mm).
        assert!(
            np_sm.neutral_point_x_mm > 20.0,
            "NP must be aft of leading edge"
        );
    }
    ...
    fn test_cg_forward_of_np_is_stable() {
        ...
        assert!(
            sm.is_stable,
            "CG forward of NP should be stable, sm={:.1}%",
            sm.static_margin_mac * 100.0
        );
        assert!(
            sm.static_margin_mm > 0.0,
            "Static margin should be positive"
        );
        assert!(matches!(
            sm.stability_category,
            StabilityCategory::VeryStable
                | StabilityCategory::Stable
                | StabilityCategory::Marginal
                | StabilityCategory::Neutral
    ...
    fn test_trim_finds_solution() {
        ...
        assert!(
            trim.trim_aoa_deg > -10.0 && trim.trim_aoa_deg < 25.0,
            "Trim AoA out of range: {}",
            trim.trim_aoa_deg
        );
        // Trim CL should be reasonable.
        assert!(
            trim.trim_cl.abs() < 2.0,
            "Trim CL out of range: {}",
            trim.trim_cl
        );
    }
}
