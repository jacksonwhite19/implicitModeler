...
pub fn compute_drag_polar(
    ...
    let cd0_fuse = if s_ref_m2 > 1e-9 {
        cf_fuse * kf_fuse * s_wet_fuse / s_ref_m2
    } else {
        0.0
    };
    ...
    let cd0_htail = if s_ref_m2 > 1e-9 {
        cf_htail * kf_htail * s_wet_htail / s_ref_m2
    } else {
        0.0
    };
    ...
    let cd0_vtail = if s_ref_m2 > 1e-9 {
        cf_vtail * kf_vtail * s_wet_vtail / s_ref_m2
    } else {
        0.0
    };
    ...
    let polar = polar_db
        .get_interpolated("NACA 0012", re_mid)
...
mod tests {
    ...
    fn test_cd0_in_range() {
        ...
        println!(
            "CD0 = {:.4}, breakdown: wing={:.4} fuse={:.4} htail={:.4} vtail={:.4}",
            result.cd0,
        ...
        assert!(
            result.cd0 >= 0.010 && result.cd0 <= 0.080,
            "CD0 should be 0.010–0.080, got {:.4}",
            result.cd0
        );
    }
    ...
    fn test_ld_max_in_range() {
        ...
        assert!(
            result.ld_max >= 5.0 && result.ld_max <= 30.0,
            "L/D max should be 5–30, got {:.1}",
            result.ld_max
        );
    }
    ...
    fn test_polar_points_populated() {
        ...
        for (cl, cd) in &result.polar_points {
            assert!(
                *cd > 0.0,
                "CD should be positive at CL={:.3}, CD={:.5}",
                cl,
                cd
            );
        }
