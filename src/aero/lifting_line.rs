...
pub fn solve_lifting_line(
    ...
    let polar = polar_db
        .get_interpolated(designation, re_mid)
...
fn chord_at_y(sdf: &dyn Sdf, y_mm: f32, bbox: &crate::sdf::query::BoundingInfo) -> f32 {
    ...
    for k in 1..=n_steps {
        ...
        if prev_d > 0.0 && d < 0.0 && x_enter.is_nan() {
            x_enter = x;
        }
        if prev_d < 0.0 && d > 0.0 {
            x_exit = x;
...
mod tests {
    ...
    fn test_stall_detection() {
        ...
        assert!(
            !res.stall_stations.is_empty(),
            "Should have stalled stations at 20°"
        );
    }
    ...
    fn test_washout_root_stall_first() {
        ...
        assert!(
            res.root_stall_first,
            "Washout should cause root to stall first"
        );
    }
}
