    ...
    if s.contains('-') {
        ...
        if parts.len() == 2 {
            ...
            if digits.len() >= 3 {
                ...
                let thickness: f32 =
                    digits[digits.len() - 2..].parse().unwrap_or(12) as f32 / 100.0;
...
mod tests {
    ...
    fn test_symmetric_zero_lift() {
        ...
        assert!(
            (p.cl_at(0.0)).abs() < 0.05,
            "CL at 0° should be ~0 for symmetric"
        );
    ...
    fn test_cambered_negative_alpha0() {
        ...
        assert!(
            p.alpha_zero_lift_deg < -1.0,
            "NACA 4412 alpha_0 should be negative"
        );
    }
    ...
    fn test_cd_positive() {
        ...
        for &cd in &p.cd {
            assert!(cd > 0.0, "CD must be positive");
        }
    }
}
