...
fn eval(script: &str) -> Arc<dyn Sdf> {
    scripting::evaluate_script(script)
...
fn test_linear_array_x() {
    ...
    for x in [0.0_f32, 10.0, 20.0, 30.0] {
        assert!(
            ...
            "should be inside copy at x={}",
            x
        );
    }
    assert!(
        sdf.distance(Vec3::new(50.0, 0.0, 0.0)) > 0.0,
...
fn test_linear_array_y() {
    ...
    for y in [0.0_f32, 8.0, 16.0] {
        assert!(
            sdf.distance(Vec3::new(0.0, y, 0.0)) < 0.0,
            "should be inside copy at y={}",
            y
        );
    }
}
...
fn test_linear_array_count_one() {
    ...
    assert!(
        (arr.distance(p) - single.distance(p)).abs() < 1e-4,
...
fn test_polar_array_6_around_z() {
    ...
    for deg in angles {
        ...
        assert!(sdf.distance(p) < 0.0, "should be inside copy at {}°", deg);
    }
}
...
fn test_example_parametric_bracket() {
    ...
    assert!(
        result.is_ok(),
        "parametric_bracket.rhai failed: {:?}",
        result.err()
    );
}
